 ...
 *
 * No hay información fiable sobre la NTC usada en el módulo KY-013 de Keyes.
...
float32_t ntc_traducir_tension_a_temperatura(float32_t V_s)
{
    ...
    float32_t ln_R_ntc;
    float32_t T;        
    const float32_t a = 1.129E-3f;  /* Coeficientes Steinhart-Hart usados */
    const float32_t b = 2.341E-4f;
    const float32_t c = 8.767E-8f;
    
    R_ntc = (10000 * V_s) / (3.3 - V_s);
		ln_R_ntc = log(R_ntc);
		T = (1 / (a + b * ln_R_ntc + c * ln_R_ntc * ln_R_ntc * ln_R_ntc)) - 273.16;
