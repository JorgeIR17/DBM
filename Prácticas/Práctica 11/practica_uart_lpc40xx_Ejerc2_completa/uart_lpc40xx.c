...
void uart_transmitir_cadena(LPC_UART_TypeDef *uart_regs, const char *cadena)
{
    uint32_t i;
		...
		ASSERT(uart_regs == UART0 || uart_regs == UART1 || 
		uart_regs == UART2 || uart_regs == UART3 || 
		uart_regs == UART4, "UART incorrecta");
	
		while(!(uart_regs->LSR & (1u << 5))) {}
			
		i = 0;
			
		while(cadena[i] != '\0')
		{
				uart_transmitir_dato(UART0, cadena[i]);
				i++;
		}
}
                         ...
                         uint32_t tamano_buffer)
{
    uint32_t i;
		...
		while(i < tamano_buffer - 2)
		{
				ptr_buffer[i] = (char) uart_esperar_recibir_dato(UART0);
				if (ptr_buffer[i] == '\r')
					continue;
				if(ptr_buffer[i] == '\n')
					  break;
